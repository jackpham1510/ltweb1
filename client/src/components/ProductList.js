...
export default ({products, categories, branchs}) => (
  <Layout.Row>
  {
    products.map(p => (
      <Layout.Col span={4}>
        <Card className="card-body d-flex fl-x-center">
          ...
          <div class="card-body__desc mt-30 bd-t-1" title={p['NAME']}>
            <h3 class="fs-14 text-dark font-weight-bold mb-5 card-body__title">{p['NAME']}</h3>
            <div className="bottom clearfix">
              <p className="fs-12 text-dark my-5">{p['SUBTITLE']}</p>
              <p className="fs-16 text-danger fw-bold my-5">{utils.formatMoney(p['PRICE'])}₫</p>
              <p className="fs-12 text-dark mt-5">
